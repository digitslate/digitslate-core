...
const char * const BITCOIN_CONF_FILENAME = "digitslate.conf";
const char * const BITCOIN_PID_FILENAME = "digitslated.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "digitslate" is a composite category enabling all DigitSlate-related debug output
            if(ptrCategory->count(string("digitslate"))) {
...
#else
    const char* pszModule = "digitslate";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DigitSlateCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DigitSlateCore
    // Mac: ~/Library/Application Support/DigitSlateCore
    // Unix: ~/.digitslate
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DigitSlateCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DigitSlateCore";
#else
    // Unix
    return pathRet / ".digitslate";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty digitslate.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override digitslate.conf
