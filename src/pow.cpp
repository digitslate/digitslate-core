...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const Consensus::Params& params) {
    ...
    uint64_t pastSecondsMin = params.GetPowTargetTimespan(BlockLastSolved->nHeight) * 0.025;
    uint64_t pastSecondsMax = params.GetPowTargetTimespan(BlockLastSolved->nHeight) * 7;
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const Consensus::Params& params) {
    /* current difficulty formula, digitslate - DarkGravity v3, written by Evan Duffield - evan@digitslate.org */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
...
unsigned int CalculateNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
    ...
    if (nActualTimespan < params.GetPowTargetTimespan(pindexLast->nHeight)/4)
        nActualTimespan = params.GetPowTargetTimespan(pindexLast->nHeight)/4;
    if (nActualTimespan > params.GetPowTargetTimespan(pindexLast->nHeight)*4)
        nActualTimespan = params.GetPowTargetTimespan(pindexLast->nHeight)*4;
    ...
    bnOld = bnNew;
    bnNew *= nActualTimespan;
    bnNew /= params.GetPowTargetTimespan(pindexLast->nHeight);
    ...
    LogPrintf("params.GetPowTargetTimespan(pindexLast->nHeight) = %d    nActualTimespan = %d\n", params.GetPowTargetTimespan(pindexLast->nHeight), nActualTimespan);
